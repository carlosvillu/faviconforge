...
export function Header({ session, user }: HeaderProps) {
  ...
  return (
    <header className={`border-b-8 border-black p-3 md:p-6 ${step ? 'bg-yellow-300' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
        ...
        <div className="flex justify-between items-center gap-2">
          <Link to="/" className="flex-shrink min-w-0">
            <h1 className="text-lg md:text-3xl font-black uppercase tracking-tight">FaviconForge</h1>
          </Link>

          {/* Mobile: Auth + Language in top row */}
          <div className="flex gap-3 md:gap-4 items-center md:hidden flex-shrink-0">
            {session && user ? (
              <UserDropdown user={user} onLogout={handleLogout} />
            ) : (
              <BrutalistButton asChild className="px-3 py-1.5 text-xs">
